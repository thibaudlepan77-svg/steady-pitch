...
async function allumer() {
  ...
  // VINGT SECONDES ET ON ABANDONNE. `getUserMedia` ne se resout jamais et ne
  // rejette jamais tant que personne ne repond a la demande de permission, donc
  // sans cette course le bouton reste sur `asking for the microphone` jusqu'a
  // la fin des temps et le visiteur croit la page morte. Un refus franc, lui,
  // rejette proprement, c'est le silence qui pose probleme.
  let garde;
  const abandon = new Promise((_, rejeter) => {
    garde = setTimeout(() => rejeter(new Error("sans reponse")), 20000);
  });
  ...
  try {
    ...
    elt("mon-erreur").textContent = !window.isSecureContext
      ...
      : erreur && erreur.message === "sans reponse"
        ? "The browser never answered about the microphone. Look for a "
          + "permission request near the address bar, allow it, then press "
          + "the button again."
        : "The browser refused the microphone. Nothing else on this page "
          + "needs it.";
    return;
  } finally {
    clearTimeout(garde);
